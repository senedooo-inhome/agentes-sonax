...
export default function AtestadosPage() {
  ...
  async function salvar(e: React.FormEvent) {
    ...
    if (!Number.isFinite(diasNum) || diasNum < 0) { alert('Informe um número válido de dias.'); return }
      ...
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        <header className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <a href="/" className="rounded-lg bg-[#2687e2] px-3 py-2 text-sm font-semibold text-white hover:bg-blue-600">Início</a>
          </div>
        </header>
        ...
        <div className="rounded-xl bg-white p-6 shadow space-y-4">
          <form onSubmit={salvar} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div>
                <label className="block text-sm font-semibold mb-1 text-[#ff751f]">Dias de atestado</label>
                <input
                  type="number" min={0}
                  className="w-full rounded-lg border p-2 text-[#535151]"
                  value={form.dias}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-semibold mb-1 text-[#ff751f]">Enviado ao RH?</label>
                <select
                  className="w-full rounded-lg border p-2 text-[#535151]"
                  value={form.enviado_rh}
                  onChange={(e)=>setForm({...form, enviado_rh: e.target.value as 'Sim'|'Não'})}
                >
                  {['Não','Sim'].map(v => <option key={v} value={v}>{v}</option>)}
                </select>
              </div>
              <div>
                <label className="block text-sm font-semibold mb-1 text-[#ff751f]">Link do arquivo (Drive, etc.)</label>
                <input
                  type="url"
                  className="w-full rounded-lg border p-2 text-[#535151]"
                  value={form.link}
        ...
        <div className="rounded-xl bg-white p-6 shadow space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            ...
            <div className="md:col-span-2">
              ...
              <input type="text" value={qNome} onChange={e=>setQNome(e.target.value)} className="w-full rounded-lg border p-2 text-[#535151]" placeholder="Digite o nome"/>
            </div>
            <div>
              <label className="block text-sm font-medium mb-1 text-[#ff751f]">Enviado ao RH</label>
              <select value={fEnviado} onChange={e=>setFEnviado(e.target.value as any)} className="w-full rounded-lg border p-2 text-[#535151]">
                {['Todos','Sim','Não'].map(v => <option key={v} value={v}>{v}</option>)}
              </select>
            </div>
          </div>
                ...
                <tbody>
                  {linhas.map(l=>(
                    <tr key={l.id} className="text-sm">
                      ...
                      <td className="border-b p-2 text-[#535151]">{l.nome}</td>
                      <td className="border-b p-2 text-[#535151]">{l.dias}</td>
                      <td className="border-b p-2 text-[#535151]">{l.enviado_rh ? 'Sim' : 'Não'}</td>
                      <td className="border-b p-2">
                        {l.link ? (
                          <a href={l.link} target="_blank" rel="noreferrer" className="text-[#2687e2] underline break-all">
                            Abrir
                          </a>
